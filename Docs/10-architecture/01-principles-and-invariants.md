...
**Ratified scope note (2026-09-01) — "mutation" means "records an actor's decision", not "stages a row".** `ensure_default_domain` (`aida.domain_service`) and `ensure_organization_integration_policy` (`aida.integration_service`) each lazily create a per-tenant default row — the line of business's `is_default` "Ungoverned" `DataDomain`, the organization's `OrganizationIntegrationPolicy` — on first read, reached from twelve `GET` routes. Both helpers take no caller-supplied value (`(session, lob)` and `(session, organization_id)` only): every field of the row they create is either a schema constant or the tenant/parent identifier already in scope, so the row is a pure function of the tenant and two different principals hitting the route in either order stage a byte-identical row. There is no decision to attribute, and naming whichever principal's request happened to arrive first as "the actor" would manufacture attribution INV-7 exists to preserve, not record it. These routes stay excused from `test_every_mutation_audits` via `_LAZY_DEFAULT_WRITE_ROUTES`, and the exclusion is falsifiable rather than declarative: `test_the_lazy_default_write_list_stays_closed` requires each excused route to still reach the helper its entry names, and `test_lazy_default_writers_record_no_actor_decision` pins both helper signatures — the moment either helper accepts a caller-supplied value (a chosen domain name, a policy override), the row starts carrying somebody's choice, this carve-out's premise is gone, and the pinned-signature test goes red. Full trade-off analysis: `Docs/review-2026-08/gap/09-inv7-audit-closeout.md` §4 (tracker `ST-18`).
