...
> `tests/test_module_scaffold_generator.py`. They have the file *names* below; the content
> behind them varies from real (`catalog`'s 576-line `service.py`) to nominal
> (`identity_tenancy`'s `service.py` is 7 lines and says "scaffold only"). Each module's
...
> (`src/atlas/modules/catalog/tests/test_module_scaffold.py` and its five siblings) run in
> CI; they were collected by nothing before.
