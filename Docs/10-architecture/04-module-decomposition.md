...
> test, and their privacy rests on the import-linter contracts that enforced it anyway.
...
**The anatomy is a target shape, not a checklist (R11-X4, 2026-09-13).** It is what a module with all of these concerns looks like. The existing modules hold only the files that have something in them: a docstring-only file promising behaviour that has not moved is removed rather than kept as a placeholder, and a module grows a file when it grows the concern.
