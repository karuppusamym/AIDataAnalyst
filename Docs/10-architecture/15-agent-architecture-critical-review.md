...
| Status | Findings |
|---|---|
| CLOSED | AR-01, AR-02, AR-07, AR-08 |
| PARTIAL | AR-03, AR-04, AR-05, AR-06, AR-10, AR-11, AR-12 |
| OPEN | AR-09 |
...
| AR-05 / P1 | Contract daily/per-run token and wall-clock caps are validated and stored. The inspected runtime checks contract existence, kill switches and selected tool slugs, but no runtime consumer of these contract cap fields was found. Model-route caps are a separate control. Token accounting is estimated. | **PARTIAL.** The caps have a runtime consumer. `aida.agent_budget` reserves against a new `agent_budget_window` row with a conditional UPDATE carrying the cap in its own `WHERE`, reconciles down to actual spend, releases in full on failure, and enforces the per-run cap twice (input estimate before the call, total after) and the wall-clock cap from the run's start. The gateway's estimator is now a named shared function, so the number a run is refused on and the number recorded against it cannot drift. **Remaining:** the PostgreSQL concurrency reproduction F05 got -- racers on separate connections at both isolation levels -- has not been run for this table; SQLite serializes writers, so the tests exercise the predicate, not contention. Provider-reported usage still does not exist; every figure is estimated, and the reconciliation call is the single place a real one would enter. Tests: `tests/test_agent_budget_and_envelope.py`. |
...
2. **Before quoting a budget cap in a governance dossier.** Reproduce the AR-05 reservation on real PostgreSQL the way F05 was reproduced: racers released from a shared barrier on separate connections, at both isolation levels. The same run answers AR-04's remaining question — the real maximum stop delay when a suspension lands mid-batch.
...
**The remediation pass.** Application code was changed; one additive migration (`a7c41e93d2b0`, `agent_budget_window`) was written and is the single alembic head, but has not been run against a live database. The Python test suite was run and passes, including tests that fail against `15f29cd` for every finding marked CLOSED or PARTIAL. No load test, no model evaluation, no PostgreSQL concurrency reproduction, no live agent decision, and no production configuration change. A green suite on SQLite is not the evidence AR-05, AR-09 or AR-11 asked for, and this document does not treat it as such.
