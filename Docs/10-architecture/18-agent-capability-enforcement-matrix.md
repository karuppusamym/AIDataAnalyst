...
| REST context-product *compile* (`GET /context-product-versions/{id}/compile`, `/compile/download`) | no | n/a | **no** | n/a | Guarded by roles only. It reads a product's governed content by version id -- the same kind of door the version reads above now gate -- and is the remaining envelope-free context-product surface. See finding 12. |
...
| 12 | `GET /context-product-versions/{id}/compile` and its `/download` read a product's governed content under roles alone, so an agent whose envelope omits a product can still compile it by version id. | Open, found 2026-09-12 while correcting row 44. The same shape as the version reads, which gate through `_enforce_capability_envelope`. |

Findings 4, 5 and 11 were closed on 2026-09-12. What keeps AR-06 open now is finding
10 -- the native tools' missing allowlist, a contract-schema change rather than a check --
and finding 12, the compile door. Both are stated with their reasons above rather than
left to be rediscovered. This page had fallen behind: it still called the reviewer gap
