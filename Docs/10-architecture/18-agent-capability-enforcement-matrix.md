...
| MCP native tools (lineage, marketplace, `validate_sql`) | **yes, since 2026-09-12** | **yes, since 2026-09-13** (`capability_envelope.native_tools`) | refused when a product scope is sent | no | R11-C6. All seven dispatched *above* the contract resolution the governed path does, so an engaged kill switch stopped nothing here. `mcp_server._native_tool_contract_denial` now resolves the contract and applies `agent_kill_blocking_reason` before the dispatch; an unresolved `agent:` identity is refused and audited (`mcp.native_tool.agent_contract_denied`). `tool_slugs` names `GovernedToolVersion` slugs and a native tool has none, so it is not read here; the native tools have their own allowlist instead, `capability_envelope.native_tools`, checked after the kill switch by `agent_contracts.native_tool_violation` and audited as `mcp.native_tool.envelope_denied` (finding 10). Five of the seven are read-only value-free metadata; `request_data_product_access` writes (a maker-checker access request) and `validate_sql` reaches the source for a dry-run estimate, which is why this is a check and not a documentation note. |
...
| REST context-product *compile* (`GET /context-product-versions/{id}/compile`, `/compile/download`) | **yes, since 2026-09-12** | n/a | **yes, since 2026-09-12** | n/a | R11-C6 finding 12. Both routes load through `context_compiler_api._load_source`, which now calls the version reads' own `_enforce_capability_envelope` -- imported, not re-implemented -- above the role, purpose and quality gates. |
