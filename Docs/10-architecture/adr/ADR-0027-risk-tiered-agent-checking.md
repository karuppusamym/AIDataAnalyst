...
> **Implementation status (2026-09-09, updated after remediation):** [Critical review AR-01 through AR-04](../15-agent-architecture-critical-review.md) found that an elevated configured ceiling admitted T2/T3 types, bulk pre-review omitted size payloads, a missing confidence yielded approval, and suspension was checked only at batch entry. **All four are now fixed and tested.** The T0/T1 boundary is enforced by `review_risk_tiers.HARD_MAX_AGENT_TIER`, which configuration cannot raise; bulk items are sized from authoritative counts and escalate when the count is unresolvable; approval requires positive object-specific evidence and abstains without it; evidence is re-derived at decision time and suspension is re-read before each commit.
>
> Three qualifications remain, and the decision stays **proposed** until they are resolved. (a) The checker is deterministic, not a second-model judgment — condition (a)'s "independent" means a different principal and a different rule set, not an independent semantic assessment, and the adversarial evaluation of what a proposal's self-reported confidence is actually worth has not been run. (b) The immediate-suspension claim is bounded at one item and only at READ COMMITTED; the multi-worker measurement of the real stop delay is outstanding. (c) Condition (b)'s oversight argument now has an enforced precondition — `reviewer_agent_max_unresolved_samples` stops the agent when the human sample queue grows unread — but the disagreement and false-approval rates that would show whether the oversight is *effective* have not been measured. Default-off configuration does not establish the live deployment state.
