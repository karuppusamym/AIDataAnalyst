...
| [0005](ADR-0005-tenancy-hierarchy.md) | Six-level enterprise isolation hierarchy | Accepted — **amended by [0018](ADR-0018-three-axis-tenancy-and-classification.md)** (0018 carries an `Amends: ADR-0005` line; 0005's own status line still reads plain `Accepted`) | Bank supplies a different legal-entity model |
...
| [0021](ADR-0021-experience-shell-stack-and-strangle-migration.md) | The experience shell is a React 18 + TypeScript + Vite application in `ui-next/`, migrated screen by screen rather than rewritten, with a read-model layer for screen-shaped reads | Accepted | None named in the ADR (see the note under the register) |
...
| [0024](ADR-0024-model-route-fallback.md) | The runtime may fall back to another approved model route on transient provider failure (429, 502, 503, 504), but only to routes named in an allow-list and already governance-approved | Accepted | Sustained fallback rate above 5% of generation requests for a week; cross-route cost drift; or a regulator requiring provider-of-record predictability per request |
| [0025](ADR-0025-auto-approve-escape-hatch.md) | Auto-approve of AI-drafted metadata is an off-by-default, per-organization, per-artifact-type policy that is itself governed, capped, audit-preserving and reversible | Proposed | Revert rate above 2% of auto-approvals per policy for two consecutive weeks (the ADR itself above 5% policy-wide), auto-disable firing more than once a quarter, regulator guidance requiring a named human approver, or a request for a fourth artifact type |
| [0026](ADR-0026-per-edge-type-lineage-review.md) | Parsed lineage edges get review-lifecycle columns on each edge table (`PROPOSED`, `ACTIVE`, `REJECTED`, `SUPERSEDED`); there is no `LineageEdge` supertype | Proposed | None named in the ADR (see the note under the register) |
...
| [0029](ADR-0029-steward-agent.md) | Task agents — steward, lineage and quality — propose under their own contracted identities on one shared runtime, and decide nothing; an agent may propose up to T2 but never decides above T1 (amended 2026-09-11) | Proposed | Either object type's acceptance rate falls below 50% over at least 50 decided proposals in 30 days, a model route is approved for drafting, the ingest side-car is proposed for inclusion, a spec other than the quality agent's asks for a proposal ceiling above T1, or a second queue is proposed for the human-only list |
| [0030](ADR-0030-api-conventions-adopt-the-shipped-contract.md) | The shipped API contract is the contract: the documented target conventions (cursor pagination everywhere, the `{"error": ...}` envelope, an `Idempotency-Key` header, `X-RateLimit-Reset`, `Deprecation` and `Sunset` headers, publish-time event payload validation, an `event_version` field) are not adopted for the current API | Accepted | The first external SDK consumer of the REST API, or a regulator or bank requirement for idempotent retries or a stable error envelope |

**Register notes (2026-09-20).**

- ADR-0021 and ADR-0026 name no revisit trigger, contrary to the rule under "How to use this register". Their rows say so rather than supply one: a trigger is the decision owner's to state, not the register's.
- ADR-0005 still reads `Accepted` in its own file although ADR-0018 amends it. Read the two together; ADR-0018 is the authority on the current tenancy shape.
