...
| [0030](ADR-0030-api-conventions-adopt-the-shipped-contract.md) | The shipped API contract is the contract: the documented target conventions (cursor pagination everywhere, the `{"error": ...}` envelope, an `Idempotency-Key` header, `X-RateLimit-Reset`, `Deprecation` and `Sunset` headers, publish-time event payload validation, an `event_version` field) are not adopted for the current API | Proposed | The first external SDK consumer of the REST API, or a regulator or bank requirement for idempotent retries or a stable error envelope |
