...
| `MetadataIngestor` | Push-only service identity: the metadata-ingestion routes (single push, batches, chunks, OpenLineage events) and nothing else. Joined the catalog in R11-AUD01 (2026-09-21); those routes had always named it and no token could carry it, so a connector or an Airflow job could only be given `MetadataAdmin` or `DataAdmin`. |
...
**Guard names and the catalog (R11-AUD01, 2026-09-21).** Nine names used to appear in `require_roles` guards without being in the catalog, so no token could carry them, and the development identity provider (which accepts any string as a role) hid it: on `POST /v1/organizations/{organization_id}/business-nodes` a `DataSteward` was refused while the ungrantable `Steward` was admitted. Now `MetadataIngestor` is in the catalog; `Steward` (the persona's name, not a role) is `DataSteward` in the three identity-module guards that named it; and the other seven (`ComplianceOfficer`, `DataEngineer`, `DataProductOwner`, `DataScientist`, `DataConsumer`, `ModelRiskManager`, `ProjectAdmin`) are removed from every guard and role list, always beside at least one grantable role, so nothing changed under OIDC. The compliance-pack routes admit `Auditor` to list, read and download (generating a pack stays with `PlatformAdmin` and `DataSteward`). The UI's default development identity is now exactly the catalog. `tests/test_guard_roles_are_grantable.py` fails if any route guard or module-level role list names a role outside `PLATFORM_ROLES`. If the bank wants a model-risk approver or a data-product owner as a distinct seat, add it deliberately as a catalog role with its own bundle; do not reintroduce a name in a guard.
...
| JWKS | Cached with TTL; refresh on unknown `kid` (at most once per 30 seconds); a failed refresh backs off 30 seconds and serves the last good key set for at most 10 minutes past its expiry, then fails closed; pinned keys supported for air-gapped operation |
...
> **Implementation status (2026-09-21).** The JWKS row above is bounded on the failure side too (R11-AUD10). A failed attempt to load the issuer's key set starts a 30-second backoff (`JWKS_REFRESH_FAILURE_BACKOFF_SECONDS`, `src/aida/oidc.py`) in which the worker makes no further attempt, of any kind, so a dead provider is asked at most twice a minute however many requests arrive. During it, tokens verify against the last good key set while it is within 10 minutes of its cache expiry (`JWKS_STALE_KEY_SET_GRACE_SECONDS`); past that, or with nothing ever loaded, verification is refused with "OIDC JWKS endpoint is unavailable". A key the issuer withdrew for compromise therefore keeps verifying for at most `oidc_jwks_cache_seconds` plus 10 minutes while the provider is unreachable (15 minutes at the shipped default) and no longer. Both are code constants, deliberately not settings. Every place that verifies a bearer token, `get_security_context` and `POST /v1/security/tokens/revoke` included, uses one verifier per process and identity-provider configuration (`shared_oidc_verifier`, `src/aida/security.py`); the revocation route used to build a new one per call and so fetched the key set every time. See `50-security/01-security-architecture.md` §4 for the trade-off. Covered by `tests/test_oidc.py` and `tests/test_token_revocation.py`.
