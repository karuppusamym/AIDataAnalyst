...
| GL-9 | AI-drafted table/column descriptions with a composite confidence score (accuracy, clarity, style, completeness), queued for maker-checker review — not auto-applied | TODO | P2 |

**GL-9, added 2026-08-30.** Atlan's "Context Agents" (Scribe, Doc) generate table/column descriptions from query history, lineage, and column names, score each draft on a composite confidence axis, and **auto-apply high-confidence output without human review** ([atlan.com/context-agents](https://atlan.com/context-agents/) — atlan.com is blocked by this environment's egress proxy; read via search-result summaries, not fetched directly). Atlas has no equivalent today — inferred *term-to-table links* carry confidence and provenance (§3, GL-8), but nothing drafts descriptive prose for an asset. The scoring mechanism is worth taking; the auto-apply-without-review policy is not, per the standing principle already recorded for Atlan in `review-2026-08/research/02-atlan.md` §8 ("mandatory human accept/reject... as non-negotiable defaults" for a bank). GL-9 is scoped as: draft + confidence score, always routed through the existing maker-checker path (§6/§7 of this module), with the score used only to set review priority/SLA — never to skip review.
