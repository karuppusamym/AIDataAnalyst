...
| VIEW / PROCEDURE | Partial — view and procedure SQL is parsed into lineage on request (LN-2) and folded into impact (LN-7). Since 2026-09-11 the lineage agent ([ADR-0029](../10-architecture/adr/ADR-0029-steward-agent.md)) also parses the view definitions and routine bodies ingestion captured, and proposes their edges for per-edge review ([ADR-0026](../10-architecture/adr/ADR-0026-per-edge-type-lineage-review.md)); a person decides every one. An approved routine edge folds into impact as a `PROCEDURE_DEFINITION` edge naming its routine, and when one routine establishes it, `get_transformation_detail` returns that routine's screened, redacted body | Measurement on a real estate |
...
| AG-13 | Lineage agent: parse captured view definitions under a contracted identity ([ADR-0029](../10-architecture/adr/ADR-0029-steward-agent.md)) | **Delivered 2026-09-11** — on demand, or scheduled once its interval is set (off by default) |
| AG-18 | Lineage agent: parse captured routine bodies, once the routine-aware table has ADR-0026's review state | **Delivered 2026-09-11** — table-to-table edges only; approved ones fold into impact |
