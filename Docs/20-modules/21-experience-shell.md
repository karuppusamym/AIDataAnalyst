...
Persona chosen in a browser dropdown is a **development convenience**, and the dropdown exists only when the identity provider is the development one. It changes the shell's presentation, not the identity or roles sent with requests. In production it is derived from identity (module 01). This matters: a persona that a user can select is a persona that grants nothing, so any capability gated on it would be a fake control.
