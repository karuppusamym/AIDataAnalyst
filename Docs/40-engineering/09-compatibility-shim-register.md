...
14 shims, 1233 shim-to-caller-file relationships across 608 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 273 | 283 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 483 | 601 | 2 |
...
- **Callers** — 273 file(s), 283 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 98, `src/atlas` 4, `tests` 165
...
- **Callers** — 483 file(s), 601 import statement(s)
  - By source root: `scripts` 12, `src/aida` 175, `src/atlas` 7, `tests` 289
