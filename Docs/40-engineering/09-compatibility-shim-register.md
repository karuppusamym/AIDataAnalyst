...
14 shims, 1152 shim-to-caller-file relationships across 558 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 311 | 316 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 257 | 267 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 445 | 556 | 2 |
...
- **Callers** — 311 file(s), 316 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 102, `src/atlas` 5, `tests` 195
...
- **Callers** — 257 file(s), 267 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 93, `src/atlas` 4, `tests` 154
...
- **Callers** — 445 file(s), 556 import statement(s)
  - By source root: `scripts` 12, `src/aida` 166, `src/atlas` 7, `tests` 260
