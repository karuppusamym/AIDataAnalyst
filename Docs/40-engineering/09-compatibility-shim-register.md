...
14 shims, 948 shim-to-caller-file relationships across 436 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 211 | 218 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 344 | 438 | 1 |
...
- **Callers** — 211 file(s), 218 import statement(s)
  - By source root: `migrations` 1, `scripts` 4, `src/aida` 79, `src/atlas` 4, `tests` 123
...
- **File** — `src/aida/models.py` (5325 lines)
...
- **Callers** — 344 file(s), 438 import statement(s)
  - By source root: `scripts` 9, `src/aida` 133, `src/atlas` 7, `tests` 195
