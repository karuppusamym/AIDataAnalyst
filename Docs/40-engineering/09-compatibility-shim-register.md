...
14 shims, 1197 shim-to-caller-file relationships across 585 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 464 | 576 | 2 |
...
- **Callers** — 464 file(s), 576 import statement(s)
  - By source root: `scripts` 12, `src/aida` 171, `src/atlas` 7, `tests` 274
...
- **File** — `src/aida/schemas.py` (4614 lines)
