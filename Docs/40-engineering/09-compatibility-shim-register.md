...
14 shims, 1130 shim-to-caller-file relationships across 540 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 305 | 310 | 2 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 433 | 543 | 2 |
...
- **Callers** — 305 file(s), 310 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 102, `src/atlas` 5, `tests` 189
...
- **Callers** — 253 file(s), 263 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 93, `src/atlas` 4, `tests` 150
...
- **Callers** — 433 file(s), 543 import statement(s)
  - By source root: `scripts` 12, `src/aida` 165, `src/atlas` 7, `tests` 249
...
- **File** — `src/aida/schemas.py` (4548 lines)
