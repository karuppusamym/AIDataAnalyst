...
14 shims, 1289 shim-to-caller-file relationships across 618 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 335 | 340 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 279 | 289 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 487 | 606 | 2 |
...
- **Callers** — 335 file(s), 340 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 104, `src/atlas` 5, `tests` 217
...
- **Callers** — 279 file(s), 289 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 99, `src/atlas` 4, `tests` 170
...
- **Callers** — 487 file(s), 606 import statement(s)
  - By source root: `scripts` 12, `src/aida` 175, `src/atlas` 7, `tests` 293
