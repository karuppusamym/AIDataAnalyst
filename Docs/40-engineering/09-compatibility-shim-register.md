...
14 shims, 1227 shim-to-caller-file relationships across 603 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 478 | 595 | 2 |
...
- **Callers** — 478 file(s), 595 import statement(s)
  - By source root: `scripts` 12, `src/aida` 173, `src/atlas` 7, `tests` 286
