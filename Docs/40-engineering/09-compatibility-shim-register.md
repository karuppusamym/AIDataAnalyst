...
14 shims, 1091 shim-to-caller-file relationships across 517 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 299 | 304 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 246 | 254 | 1 |
| [`aida.context`](#aidacontext) | python | `atlas.platform.context` | Platform infrastructure | 68 | 68 | 0 |
| [`aida.logging`](#aidalogging) | python | `atlas.platform.logging` | Platform infrastructure | 5 | 5 | 0 |
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 411 | 517 | 2 |
| [`aida.schemas`](#aidaschemas) | python-partial | `atlas.modules.<context>.schemas` (re-exported DTOs only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 21 | 21 | 0 |
| [`aida.catalog_read_model`](#aidacatalogreadmodel) | python | `atlas.modules.catalog.service` / `.repository` | Bounded context: catalog | 11 | 11 | 0 |
...
- **Callers** — 299 file(s), 304 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 100, `src/atlas` 5, `tests` 185
...
- **Callers** — 246 file(s), 254 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 90, `src/atlas` 4, `tests` 146
...
- **Callers** — 68 file(s), 68 import statement(s)
  - By source root: `src/aida` 64, `src/atlas` 4
...
- **Callers** — 411 file(s), 517 import statement(s)
  - By source root: `scripts` 12, `src/aida` 154, `src/atlas` 7, `tests` 238
...
- **File** — `src/aida/schemas.py` (4309 lines)
...
- **Callers** — 21 file(s), 21 import statement(s)
  - By source root: `src/aida` 5, `src/atlas` 4, `tests` 12
- **Named in import-linter contracts** — `catalog module privacy`, `connectivity module privacy`, `identity_tenancy module privacy`, `ingestion module privacy`, `observability_audit module privacy`, `profiling module privacy`
...
- **Callers** — 11 file(s), 11 import statement(s)
  - By source root: `src/aida` 9, `tests` 2
