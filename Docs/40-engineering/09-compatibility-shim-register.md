...
14 shims, 1148 shim-to-caller-file relationships across 554 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 308 | 313 | 2 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 444 | 555 | 2 |
...
- **Callers** — 308 file(s), 313 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 102, `src/atlas` 5, `tests` 192
...
- **Callers** — 444 file(s), 555 import statement(s)
  - By source root: `scripts` 12, `src/aida` 166, `src/atlas` 7, `tests` 259
...
- **File** — `src/aida/schemas.py` (4590 lines)
