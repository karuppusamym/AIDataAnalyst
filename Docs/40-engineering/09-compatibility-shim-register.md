...
14 shims, 1205 shim-to-caller-file relationships across 592 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 326 | 331 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 268 | 278 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 469 | 584 | 2 |
...
- **Callers** — 326 file(s), 331 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 104, `src/atlas` 5, `tests` 208
...
- **Callers** — 268 file(s), 278 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 97, `src/atlas` 4, `tests` 161
...
- **Callers** — 469 file(s), 584 import statement(s)
  - By source root: `scripts` 12, `src/aida` 171, `src/atlas` 7, `tests` 279
...
- **File** — `src/aida/schemas.py` (4618 lines)
