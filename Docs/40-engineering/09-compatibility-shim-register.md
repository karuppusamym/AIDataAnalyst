...
14 shims, 1229 shim-to-caller-file relationships across 604 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 329 | 334 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 272 | 282 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 480 | 597 | 2 |
| [`aida.schemas`](#aidaschemas) | python-partial | `atlas.modules.<context>.schemas` (re-exported DTOs only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 26 | 27 | 0 |
...
- **Callers** — 329 file(s), 334 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 104, `src/atlas` 5, `tests` 211
...
- **Callers** — 272 file(s), 282 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 98, `src/atlas` 4, `tests` 164
...
- **File** — `src/aida/models.py` (5463 lines)
...
- **Callers** — 480 file(s), 597 import statement(s)
  - By source root: `scripts` 12, `src/aida` 175, `src/atlas` 7, `tests` 286
...
- **File** — `src/aida/schemas.py` (4701 lines)
...
- **Owner area** *(hand-written)* — Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly
- **Introduced by** — ST-05, Phase 3 of Docs/40-engineering/06-refactor-plan.md
- **Re-exports** — 80 name(s) from `atlas.modules.catalog.schemas`, `atlas.modules.connectivity.schemas`, `atlas.modules.identity_tenancy.schemas`, `atlas.modules.ingestion.schemas`, `atlas.modules.observability_audit.schemas`, `atlas.modules.profiling.schemas`
- **Callers** — 26 file(s), 27 import statement(s)
  - By source root: `src/aida` 7, `src/atlas` 4, `tests` 15
- **Named in import-linter contracts** — `catalog module privacy`, `connectivity module privacy`, `identity_tenancy module privacy`, `ingestion module privacy`, `observability_audit module privacy`, `profiling module privacy`
...
- **Callers** — 12 file(s), 12 import statement(s)
  - By source root: `src/aida` 10, `tests` 2
...
- **Callers** — 2 file(s), 2 import statement(s)
  - `src/aida/main.py`, `tests/test_access_change_review.py`
- **String references** — 2 file(s): `scripts/generate_architecture_map.py`, `tests/test_inv4_authorization_wiring.py`
- **Named in import-linter contracts** — `R11-GQL01 GraphQL's read modules and the shared context-product and OKF read services never import a router`, `identity_tenancy module privacy`
...
- **Named in import-linter contracts** — `R11-GQL01 GraphQL's read modules and the shared context-product and OKF read services never import a router`, `ingestion module privacy`
...
- **String references** — 1 file(s): `scripts/generate_architecture_map.py`
- **Named in import-linter contracts** — `R11-GQL01 GraphQL's read modules and the shared context-product and OKF read services never import a router`, `observability_audit module privacy`
