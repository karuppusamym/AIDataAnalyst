...
14 shims, 1173 shim-to-caller-file relationships across 572 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 454 | 565 | 2 |
...
- **Callers** — 454 file(s), 565 import statement(s)
  - By source root: `scripts` 12, `src/aida` 170, `src/atlas` 7, `tests` 265
