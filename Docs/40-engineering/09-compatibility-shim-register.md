...
14 shims, 1315 shim-to-caller-file relationships across 631 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 292 | 302 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 492 | 614 | 2 |
...
- **Callers** — 292 file(s), 302 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 101, `src/atlas` 4, `tests` 181
...
- **Callers** — 492 file(s), 614 import statement(s)
  - By source root: `scripts` 12, `src/aida` 175, `src/atlas` 7, `tests` 298
