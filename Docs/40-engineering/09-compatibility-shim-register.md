...
14 shims, 1186 shim-to-caller-file relationships across 578 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 321 | 326 | 2 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 458 | 569 | 2 |
...
- **Callers** — 321 file(s), 326 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 104, `src/atlas` 5, `tests` 203
...
- **Callers** — 458 file(s), 569 import statement(s)
  - By source root: `scripts` 12, `src/aida` 171, `src/atlas` 7, `tests` 268
