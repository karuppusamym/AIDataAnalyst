...
14 shims, 1278 shim-to-caller-file relationships across 613 distinct files, 0 shim(s) with a measured caller count of zero.

| Shim | Kind | Replacement path | Owner area | Callers | Import stmts | String refs |
|---|---|---|---|---:|---:|---:|
| [`aida.db`](#aidadb) | python | `atlas.platform.db` | Platform infrastructure | 331 | 336 | 2 |
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 275 | 285 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 484 | 602 | 2 |
...
- **Callers** — 331 file(s), 336 import statement(s)
  - By source root: `migrations` 1, `scripts` 8, `src/aida` 104, `src/atlas` 5, `tests` 213
...
- **Callers** — 275 file(s), 285 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 99, `src/atlas` 4, `tests` 166
...
- **Callers** — 484 file(s), 602 import statement(s)
  - By source root: `scripts` 12, `src/aida` 175, `src/atlas` 7, `tests` 290
...
- **File** — `src/aida/schemas.py` (4728 lines)
