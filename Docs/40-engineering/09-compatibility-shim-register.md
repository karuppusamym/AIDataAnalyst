...
14 shims, 1202 shim-to-caller-file relationships across 590 distinct files, 0 shim(s) with a measured caller count of zero.
...
| [`aida.config`](#aidaconfig) | python | `atlas.platform.config` | Platform infrastructure | 266 | 276 | 1 |
...
| [`aida.models`](#aidamodels) | python-partial | `atlas.modules.<context>.models` (re-exported classes only) | Bounded contexts (catalog, connectivity, identity_tenancy, ingestion, observability_audit, profiling) jointly | 468 | 583 | 2 |
...
- **Callers** — 266 file(s), 276 import statement(s)
  - By source root: `migrations` 1, `scripts` 5, `src/aida` 97, `src/atlas` 4, `tests` 159
...
- **Callers** — 468 file(s), 583 import statement(s)
  - By source root: `scripts` 12, `src/aida` 171, `src/atlas` 7, `tests` 278
