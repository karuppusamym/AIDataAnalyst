...
- **The sample-to-correction link covers bulk stewardship and enrichment annotations, not descriptions.** A description a steward withdraws through `DESCRIPTION_WITHDRAWAL`, or a re-published draft, still carries no reference to the sample that prompted it, so for the two description draft types "was this disagreement acted on?" is still answered by reading the object's history.
