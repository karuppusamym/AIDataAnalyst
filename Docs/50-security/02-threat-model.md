...
> **Implementation status (2026-09-20).** Two facts about the role model bear on T1 and T19. First, the role catalog is sixteen names (`PLATFORM_ROLES` in `src/aida/oidc.py`), and since R11-AUD01 (2026-09-21) `tests/test_guard_roles_are_grantable.py` fails if a route guard or a role list names anything else; nine names used to appear in guards that no OIDC token could carry, so a role gate behaved differently in development, where the identity accepts any string, than in production. `PlatformAdmin` is cross-tenant by design. Second, the two access changes that `src/aida/review_risk_tiers.py` classes as the highest review risk tier (the trust boundary itself), creating an access policy and adding a workspace member, used to be a single call by an admin role. Since R11-AUD02 each opens a governance review that a different principal decides, so T19's "maker-checker on governed changes" covers them; both are described, with the routes, in `20-modules/01-identity-and-tenancy.md` §5a and §10.
