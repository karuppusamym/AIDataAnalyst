...
| `redis_url` | destination | `localhost:6379` over `redis` | localhost default (host only shown) | yes -- local only, not an external destination | `aida.request_budget`, `aida.unified_lineage_api` | no external target -- localhost default only | no approval gate found -- whoever sets the variable decides | no enabling flag -- used whenever read | no readiness probe | unknown |
