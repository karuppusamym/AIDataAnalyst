...
confirmation is the [2026-09-22 FP08/FP14/FP15 closure](32-fp08-fp14-fp15-closure-2026-09-22.md), on
...
Most scheduled implementation has landed: **95 DONE, 24 PARTIAL, 7 BLOCKED, 0 TODO, 22 DEFERRED,
2 CANCELLED** across 150 unique work packages. Of the 126 that are neither deferred nor cancelled,
95 are complete; 31 still need implementation, verification or prerequisites. **32 of the 33
...
second way to do what the UI's own flows already do. Three more PARTIAL rows closed the same day
with no code change: FP08, FP14 and FP15 each had nothing left to build — a stale cross-reference
the code had already resolved (FP08), or a decision against building already recorded elsewhere
(FP14, FP15).
...
| Agent orchestration | 13 | Partial | Framework-neutral typed states including a pre-retrieval `SCREENED` gate; versioned deterministic prompt-risk classifier blocking instruction override, prompt/credential extraction, policy/masking bypass, privilege escalation, unbounded extraction, and (2026-09-02, AG-3) bank-governance attacks — maker-checker/dual-control bypass, AML/KYC/sanctions-hold override, audit and regulatory-reporting suppression; governed retrieval; approved-tool-first execution; bounded grounding; OpenAI/Gemini structured output; semantic/policy pinning; value-free trace and plan evidence; bank model-risk evaluation corpus + harness with published zero-bypass/zero-false-positive accuracy-and-refusal results (AG-3, `Docs/90-reference/model-risk-benchmark-results.md`) | Multi-step tool plans; indirect-injection defences; multilingual/obfuscation coverage; retrieval/model benchmarks; generation-quality benchmarking (needs a live, approved model route this sandbox does not have) |
...
| Model route and AI governance | 15 | Partial | Immutable route versions with residency/retention/capability/budget contracts; credential-reference redaction; maker-checker lifecycle; honest activation states; OpenAI Responses and Gemini adapters with structured output, bounded retries/timeouts, non-content evidence; durable evaluations; bank model-risk evaluation corpus + harness (MG-5, same evidence as module 13's AG-3, `Docs/90-reference/model-risk-benchmark-results.md`) | Rotated credentials; bank-approved route selection; private routing/workload identity; retention certification; monitoring; **kill-switch drill** |
| Data-quality observability | 11 | Implemented for profile-baseline controls | Source/table policies; deterministic volume, null-rate, schema-fingerprint comparison; immutable value-free observations; fingerprinted incident lifecycle with auto-recovery; audited acknowledge/resolve; metadata scan-age posture; automatic Temporal integration; Atlas investigation workspace; **maker-checker-approved watermark contracts genuinely activate freshness evaluation** (`POST .../freshness-config/{table_id}/approve`, self-approval blocked — DQ-2, 2026-09-02); **notification/escalation routing is wired to real quality incidents**, persisting `NotificationEventRecord` rows and, for ITSM-channel matches, performing a real (off-by-default, configurable-URL) webhook POST (`quality_service.route_and_notify_incident`/`emit_itsm_webhook` — DQ-1, 2026-09-02); custom rule packs scheduled outside scans (DQ-4); runtime coupling into tool gating/answer trust/retrieval ranking (DQ-3) | SLOs; bank-scale incident certification; EMAIL/generic-WEBHOOK channel delivery transport (persisted as routed, actual send is an infra concern) |
| Policy and governance | 17 | Implemented for the wired query path | RBAC role gates, organization enforcement, unified cross-object review queue with filters, rationale capture, independent decisions, platform-enforced maker≠checker; ABAC (classification/purpose/residency, DENY as a hard ceiling) wired into the real execution/validation path with a measured perf benchmark target (`policy_engine.py`, PG-1); agent-vs-human (`principal_kind`) as a first-class subject attribute (PG-2); bulk governance decisions with per-item rationale at up to 10,000 items (PG-3); delegation/reassignment with self-approval-by-proxy blocked (PG-4); edition entitlement gating (PG-5); full policy decision logging via the general audit trail (PG-6); policy simulation (PG-8) | External PDP (OPA / bank PDP) adapter certified against the bank bundle (PG-7) |
