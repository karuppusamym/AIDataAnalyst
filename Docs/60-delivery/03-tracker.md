...
| CX-9 | Marketplace personalization by role/domain | 19 | C | P2 | IN PROGRESS | — | New 2026-08-30 (from Atlan Data Marketplace review, `Docs/20-modules/19-context-products-and-mcp.md` CP-4 addendum). `product_marketplace_api.py` listing today returns the identical unfiltered/unranked catalog to every requester; rank/default-filter by the requester's owned domains (GL-2/GL-6 ownership) and role so an analyst and a business user see a relevant subset of the same estate, not the same list. Exit: two requesters with different domain ownership get demonstrably different default orderings from the same listing endpoint, proven by a test |
