...
| RL-2 | Canonical table resolution | 06 | B | P1 | DONE | — | Delivered 2026-08-30. Evidence-backed grouping (exact bare-name match or near-identical column signature/fingerprint) of `MetadataTable` rows believed to be the same logical entity across schemas/catalogs/datasources; a default canonical member is suggested but only ever set on a steward's own maker-checker `APPROVE`, which may override the default (`canonical_table_resolution.py`, `canonical_table_api.py`). Exposed as a standalone, queryable table for future retrieval-ranking consumers rather than wired into the ranking pipeline itself (avoided touching `retrieval.py`/`fusion_ranking.py`, both hot from today's RT work) |
