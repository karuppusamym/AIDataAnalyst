...
| GL-9 | AI-drafted asset descriptions, evidence-scored, maker-checker gated | 08 | C | P2 | IN PROGRESS | — | New 2026-08-30 (from Atlan competitive review, `Docs/20-modules/08-glossary-and-stewardship.md`). Draft description generated from real signals (lineage, column/type shape, dbt description, bound glossary terms via SM-2) with a composite confidence score; always enters the existing maker-checker queue — score sets review priority, never bypasses review. Exit: end-to-end test proving a low-signal asset cannot auto-publish and a high-confidence draft still requires independent approval |
...
| ST-A8 | Usage-derived answer eval suite as a change-set gate | 18 | C | P1 | IN PROGRESS | — | New 2026-08-30 (from Atlan Context Engineering Studio review, `Docs/20-modules/18-studio.md`). Mines real query/consumption history and BI lineage for questions a semantic object/tool answered, persists them as a regression corpus, and re-checks them on every change set touching that object — deterministic resolution/compilation regression, not free-text LLM grading (keeps the value-free, ADR-0014-consistent shape). Exit: a change set that breaks a previously-passing mined question is blocked at the existing test gate (`studio.py::run_tests`), proven by a test that mutates a metric and shows the gate catching it |
