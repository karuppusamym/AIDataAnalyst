...
| Other connectors | 02 | Pending | BigQuery, Snowflake, Databricks, Teradata, Db2 are visibly `PLANNED` | Implement and certify in bank priority order |
...
| SQL / query lineage | 09 | Partial | Durable referenced tables/columns, value-free SELECT output-to-source mappings, direct/derived classification, transformation names, governed-tool dependencies, API and Atlas visibility | View/procedure lineage; ETL/OpenLineage; warehouse history adapters |
| dbt transformation intelligence | 09 | Partial | Governed project registration, immutable manifest v12-compatible ingestion, model/source/test inventory, catalog matching, SQL hash plus literal-redacted SQL, dependency DAG, impact, agent retrieval, Atlas workbench | CI/dbt Cloud auth; `run_results.json`; column-level lineage; retention; large-DAG virtualization |
...
| Context products and MCP | 19 | **Pending** | — | Entire module: MCP server, context products, per-read policy, consumption lineage, eligible tools |
...
2. **Two modules are entirely pending** — studio (18) and context products (19). Glossary (08) now implements the governed table-stewardship slice; broader asset types, operational automation, and bank-scale certification remain.
