...
| **P0** | Context products and MCP | JSON-RPC 2.0 MCP server; immutable maker-checker Context Product versions; normalized role bindings; lifecycle and quality-gated REST/MCP reads; exact product-scoped tool exposure; immutable consumption edges; native bounded lineage graph/impact tools; audit/outbox evidence and UI control plane | Purpose ABAC; generic-resource and native-lineage consumption edges; per-consumer budgets; `prompts/*`; fuzzy resolution; transformation-detail tool; governed context compiler; data product/contract registries; marketplace; unified AI registry and trust scoring — full CP-1..CP-14 detail in `20-modules/19-context-products-and-mcp.md` §15.2 |
...
| P1 | **Lineage MCP tools** (CP-6 / EE.10) | **Partially closed 2026-08-29**: `atlas__get_lineage_graph` and `atlas__get_lineage_impact` are now native MCP tools. Still open: fuzzy asset resolution (`MCP-3`), transformation-detail-as-a-tool, and consumption-lineage recording (`CX-4`) — `competitors/08-collibra-lineage-and-platform-analysis-2026-08.md`, `competitors/09-collibra-marketplace-catalog-integrations-mcp-governance-2026-08.md` |
| P1 | **MCP write operations** (`MCP-2`) | Collibra's MCP server lets an agent create/edit assets, propose glossary terms, and manage classification matches, all through the same permission model; ours is read + governed-SQL-execution only — `competitors/09-...-mcp-governance-2026-08.md` |
| P1 | **Fuzzy entity resolution for MCP tools** (`MCP-3`) | Every MCP tool we expose (governed SQL, catalog, and the new lineage tools) requires an exact UUID; Collibra's MCP resolves names like "customers" to the right asset |
| P2 | **Governed context compiler** (CP-5 / EE.9) | Without it, semantic definitions cannot be delivered as Snowflake Semantic Views / Databricks Metric Views / ODCS, which is now table stakes for warehouse-native AI consumers |
| P2 | **Data product and data contract registries** (CP-2, CP-3 / EE.8) | `data_contracts.py` validates ingestion envelopes only; there is no product/contract lifecycle, versioning, or marketplace surface |
| P2 | **Unified AI registry and trust scoring** (CP-7, CP-8 / EE.11) | No inventory of AI use cases/agents/models exists outside `AgentRun` execution rows; no explainable trust score |
