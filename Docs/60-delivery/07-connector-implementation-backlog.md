...
| Governed execution | **Adapter code done; not reachable through the gateway** — `execute_read_query` captures a real session identifier via `SYS_CONTEXT('USERENV','SID')`, recorded as `oracle-sid:<sid>`, matching the SQL Server (`sqlserver-spid:`) and PostgreSQL (`postgres-backend:`) convention of a real backend-scoped identifier. But with `explain=False` (next row) the gateway refuses every Oracle statement (`QUERY_ESTIMATE_UNAVAILABLE_FOR_CONNECTOR`, `src/aida/sql_validation.py`), so the engine capability matrix marks Oracle TABLE and VIEW execution `UNSUPPORTED` (`NO_QUERY_ESTIMATE`) |
...
| 6 | Governed execution | **Done** — read-only query submission, `.result(timeout=...)`, BigQuery job ID captured as `warehouse_query_id` as the bare `job.job_id` (`src/aida/connectors/bigquery.py`) — there is no `bigquery-job:` prefix, unlike the `oracle-sid:`/`sqlserver-spid:`/`postgres-backend:` identifiers. Cancel handling is **not** implemented — no connector implements it yet (tracked as QG-4) |
...
| 5 | Governed execution | **Done** — read-only execution capturing the real Snowflake query ID via `cur.sfqid` as the bare `warehouse_query_id` (`src/aida/connectors/snowflake.py`) — there is no `snowflake-query:` prefix, unlike the `oracle-sid:`/`sqlserver-spid:`/`postgres-backend:` identifiers |
...
| Databricks | ✅ (`information_schema.views`) | ✅ (`information_schema.routines`/`.parameters`) | ✅ (the comment columns on the discovery queries) | ❌ Unity Catalog's privilege model is not the SQL grant model: `TABLE_PRIVILEGES` reports only the current principal's own grants, so a read would look like a full inventory while being one principal's row |
| PostgreSQL | ✅ | ✅ | ✅ | ✅ |
| SQL Server | ✅ | ✅ | ✅ | ✅ |

Databricks (`src/aida/connectors/databricks.py`, registered `IMPLEMENTED`/`BETA`, PAT auth only, EXPLAIN COST estimate, never run against a live workspace) reads object comments from the adapter's first version and reached the views and routines axes in R11-FP01; it is not covered by the Oracle, Snowflake and BigQuery workstream sections above, and its `triggers` and `sequences` read `NOT_APPLICABLE` because Unity Catalog has neither object.
