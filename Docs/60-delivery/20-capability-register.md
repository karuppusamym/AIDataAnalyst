...
| A stored procedure or function is reachable by a question | Yes. ROUTINE candidate in the lexical stage, with graph edges to its reviewed tables; the SQL model gets the tables, never the routine; MCP `resolve_entity` covers ROUTINE | Yes. Inside `hybrid_retrieve` and `hybrid_retrieve_enhanced` on the live Ask path, and in MCP | n/a | **Partial.** In-memory SQLite, including graph expansion end to end; not measured against the R11-B2 answer corpus | 2026-09-15 | Engineering | `src/aida/retrieval.py`; `src/aida/retrieval_stages.py`; `tests/test_retrieval_routines.py` |
| A context product binds the approved ontology meaning it was built on | Yes. `context_product_version.ontology_version_ids`, validated to APPROVED versions of the product's organization and pinned by version id; omitted from the fingerprint and compiled references while empty | Yes. Create draft's ontology-version picker; compile, download, drift and the MCP resource read carry the bound ids | n/a (nothing is bound until an author picks a version) | **Partial.** In-memory SQLite and pure compiler tests; migration verified up/down/up on scratch PostgreSQL. Retrieval inside a product does not yet prefer the bound version over the published pointer | 2026-09-15 | Engineering | `src/aida/context_product_api.py`; `src/aida/context_compiler.py`; `tests/test_context_product_ontology_binding.py`; `ui-next/src/screens/ContextProductDraft.tsx` |
