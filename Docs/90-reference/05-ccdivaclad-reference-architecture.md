...
| Graph exploration and saved views | Current Lineage workflow | There are no saved perspectives to build on: the `graph_perspectives_api.py` that once persisted them was never wired to a screen and was removed by R11-X5. Add a focused impact perspective and evidence drawer instead of introducing a second graph viewer. | CA-11 |
