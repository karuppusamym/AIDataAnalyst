...
| PROCEDURE | parsing_lineage | PARTIAL | `PARSER_DEGRADES_EXPLICITLY` | bigquery, oracle, postgres, snowflake, sqlserver | procedure_lineage explicitly degrades on 7 of 36 recognised constructs, each producing a named UNPARSED marker rather than a silent drop; per-object completion is recorded on routine_parse_coverage, so 'inventoried' is never read as 'understood' |
...
| FUNCTION | parsing_lineage | PARTIAL | `PARSER_DEGRADES_EXPLICITLY` | bigquery, oracle, postgres, snowflake, sqlserver | procedure_lineage explicitly degrades on 7 of 36 recognised constructs, each producing a named UNPARSED marker rather than a silent drop; per-object completion is recorded on routine_parse_coverage, so 'inventoried' is never read as 'understood' |
