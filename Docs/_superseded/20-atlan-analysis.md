...
### An Atlan strength we don't yet match: distribution, not just depth

*(Added 2026-08-30, corroborated by an independent practitioner review — see `90-reference/03-sources.md` → "Independent practitioner reviews".)* Atlan markets itself first as a **collaboration hub for data teams**, not a catalog: Slack-native notifications and a Chrome extension surfaced over BI tools are core to how it drives adoption, on top of the Context Interfaces already diagrammed in §2 (row `Ext[Chrome & Slack Extensions]`). Atlas today is a single-page portal (`ui/`) — nobody sees a governance event, a quality incident, or an answer unless they open it. This is an adoption/UX gap, not a governance-depth gap, and it is tracked as whitespace item **W11** in `00-product/05-differentiation-and-whitespace.md` rather than folded into the matrix, since it doesn't change any `●/◐/○` capability score.
