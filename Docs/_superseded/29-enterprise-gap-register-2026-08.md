...
| **P0** | Model route and AI governance | Provider-neutral structured gateway; bounded metadata grounding; OpenAI and Gemini adapters; bounded retry/timeout/token contracts; durable control evaluations; pre-retrieval deterministic prompt-risk gate; versioned maker-checker routes with opaque credential references; **indirect injection defense with pattern detection, multilingual and encoding-aware corpus** (`injection_defense.py`, `injection_corpus.py`). Generation stays fail-closed until an approved route is selected and its credential resolves | Rotate development keys; approve provider/model/route selection; replace environment credentials with workload identity and private routing; certify retention/residency; bank-domain evaluations; connect monitoring; **exercise the kill switch** |
...
| **P0** | Retrieval breadth | **CLOSED 2026-08-30.** Full-text index (GIN, ts_query, cross-source), vector retrieval (pgvector, cosine similarity), graph expansion (BFS, org-boundary), fusion ranking (reciprocal rank, weighted linear), global search API + command palette UI, and enhanced hybrid retrieval orchestration all implemented. Files: `full_text_index.py`, `vector_retrieval.py`, `graph_retrieval.py`, `fusion_ranking.py`, `search_api.py`, `retrieval.py` | Large-catalog benchmarks remain |
...
| P1 | Data-quality observability | **Substantially closed 2026-08-30.** Deterministic value-free baselines, source/table policies, immutable observations, deduplicated durable incidents, recovery reconciliation, scan age, audited operator transitions, Atlas workbench; **freshness monitoring with watermark config and maker-checker** (`freshness.py`, `quality_api.py`); **quality-runtime coupling with demotion, trust warnings, tool gating** (`quality_coupling.py`); **trust scoring with composite 0-100, A-F grade** (`trust_scoring.py`); **runtime data contracts with schema drift, quality breach, SLA** (`runtime_contracts.py`, `runtime_contracts_api.py`); **notification routing with rules, escalation, dedup, ITSM** (`notification_routing.py`, `notification_api.py`) | Custom rule packs; seasonality; incident-volume/load tests; induced anomaly and recovery certification |
...
| P1 | Operations and compliance | **CLOSED 2026-08-30.** Structured logs, metrics, audit/outbox, fleet evidence, retry/backoff, dead-letter visibility, requeue control; **OpenTelemetry tracing and metrics** (`observability.py`); **SIEM routing with CEF format, syslog/webhook transport** (`siem_routing.py`); **WORM archive with immutable audit, legal hold, retention** (`worm_archive.py`); **SLO definitions with error budgets** (`observability_api.py`); **compliance pack generation for 5 frameworks** (`compliance_packs.py`, `compliance_api.py`) | Bank-specific SIEM endpoint and WORM storage configuration remain bank decisions |
...
| P1 | Studio | **Substantially closed 2026-08-30.** Form-based authoring in the portal; **change sets with create, items, conflict detection** (`studio.py`, `studio_api.py`); **test harness with fixture validation and metrics** (`studio_test_harness.py`); **diff view and impact preview** (`studio_api.py`) | Parameter designer, Git binding remain |
...
| P0 | **Indirect prompt injection through retrieved metadata** | **CLOSED 2026-08-30.** Indirect injection defense with pattern detection, multilingual and encoding-aware corpus implemented (`injection_defense.py`, `injection_corpus.py`). Original concern: a malicious column description reaching model context bypasses the pre-retrieval screen (threat T7 residual) |
| P1 | **MCP consumer threat surface** | **Substantially closed 2026-08-30.** Per-read policy evaluation wired into `mcp_server.py` (CX-3), per-consumer rate limits via `mcp_budget.py` (CX-6), and consumption lineage with consumer tracking (`consumption_lineage.py`, `consumption_lineage_api.py`; CX-4) all implemented. Workload identity remains (threat T18) |
...
| P1 | **Legal hold** | **CLOSED 2026-08-30.** WORM archive with legal hold support implemented (`worm_archive.py`) |
...
| P1 | **Lineage MCP tools** (CP-6 / EE.10) | **Closed 2026-08-29**: `atlas__get_lineage_graph`, `atlas__get_lineage_impact`, `resolve_entity` (fuzzy resolution), and `get_transformation_detail` are all native MCP tools, successful calls record audit/outbox consumption evidence, and `tests/test_mcp_server.py` now covers the two newest tools' validation, anti-enumeration denials, success payloads, and not-found behavior. Dedicated-environment scale certification remains tracked under the broader lineage and Context Product rows — `competitors/08-collibra-lineage-and-platform-analysis-2026-08.md`, `competitors/09-collibra-marketplace-catalog-integrations-mcp-governance-2026-08.md` |
...
| P2 | **Unified AI registry and trust scoring** (CP-7, CP-8 / EE.11) | **Closed foundation 2026-08-29**: lifecycle/assessment/trust APIs, managed EU AI Act/NIST/AI-UC templates, remediation, maker-checker retirement, provider evidence sync, immutable score history, dependency graph APIs, and the portal dependency-graph visualization are implemented; external registry adapters remain |
