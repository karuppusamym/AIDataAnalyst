...
| **pgvector** | **Superseded by [ADR-0019](../../10-architecture/adr/ADR-0019-vector-index-without-pgvector.md)** | Naming `pgvector` was a mistake: the target estate has no `vector` extension and will not get one, because `CREATE EXTENSION` needs a privilege a bank DBA will not grant a new platform. What the design needs is *an embedding store with nearest-neighbour search*, which is a port. Default adapter is exact cosine over `bytea` in plain PostgreSQL; the bank's in-network vector service and `pgvector` sit behind the same interface. Measured envelope: 45 ms at 200 candidates, 100 ms at 1,000, 427 ms at 5,000 — so two-stage retrieval, always. |
...
| **Neo4j** | **Drop — now on evidence, see [ADR-0020](../../10-architecture/adr/ADR-0020-graph-store-decision.md)** | The original argument here was a cost argument and was too glib about capability: a graph database *is* better at deep, variable-length paths, and lineage is where depth is real. So it was measured. On PostgreSQL 16, a 12-layer column-level DAG with 880,000 edges traverses 12 hops upstream from a report column in **10.8 ms**, reaching 3,637 nodes. The join-per-hop cost does not bite at this depth. Still unmeasured and honestly flagged: hub-shaped fan-out, all-paths enumeration, and graph algorithms — each a stated reversal condition rather than a hand-wave. |
