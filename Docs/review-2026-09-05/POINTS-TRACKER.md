...
|---|---|
| `ruff check .` | pass |
| `mypy src sdk/aida_tool_sdk` | pass, 366 source files |
| `lint-imports` | 11 contracts kept, 0 broken (3 added across this remediation) |
| `alembic heads` | single head, `c3f0a71d5e94` |
| `pytest tests/` | see the run recorded in the commit message |
| `test_migration_orm_drift.py` | **ran against real PostgreSQL 17, not skipped** — this is R04's proof that relocating a bounded context changed no schema |
| `test_governance_decision_postgres_concurrency.py` | 14 passed against real PostgreSQL — F05's reproduction |
| `scripts/openapi_diff.py` | no breaking changes; **zero classified changes of any kind** after R04, which is the stronger claim that relocation moved no DTO |
...
| `scripts/check_npm_audit.py` | pass — 14 dev-only advisories baselined, runtime set clean |
| `scripts/check_frontend_reachability.py` | pass — 3 unreachable modules, all allow-listed with reasons |
| `scripts/check_docs_links.py` | pass, 213 Markdown files |
| `scripts/check_proxy_contract.py`, `check_image_packaging.py` | pass |
| `shim_register.py`, `generate_architecture_map.py`, `generate_surface_control_matrix.py`, `generate_destination_inventory.py` | all `--check` clean |
| `tsc --noEmit`, `vitest run`, `vite build` (ui-next) | pass, **511 tests** |
...
|---|---|---|---|---|
| F01 | P0 | WORM archival reports completion without storing the archive | ☑ / ⚠ | Two-phase PREPARED→UPLOADED→VERIFIED→FAILED lifecycle; progress advances only past bytes a destination acknowledged **and** handed back intact. `FilesystemArchiveStorage` is complete (fsync, read-only mode bit, sidecar manifest, read-back verification, retention, legal hold). `NullArchiveStorage` is the **default** and refuses. **`S3ArchiveStorage` now exists and Object Lock is exercised for real** against a running MinIO, with SigV4 signed by hand (no `boto3`) and pinned to four of AWS's published known-answer vectors. COMPLIANCE mode, deliberately: GOVERNANCE is waivable by anyone holding `s3:BypassGovernanceRetention`. Proven: retrieve + re-checksum; delete before retain-until refused **by the service** (400 `InvalidRequest`, "Object is WORM protected"); legal hold outranks lapsed retention, with hold-release-then-delete-succeeds as the control. **Overwrite is not refused** — Object Lock requires versioning, so a raw PUT becomes a new version and the locked one survives; every URI therefore pins a `versionId`. `gcs`/`azure_blob` remain explicitly unavailable, with a test that they are not quietly given S3's behaviour. ⚠ **MinIO is not AWS**: refusal codes differ (400 vs 403), IAM/bucket policy were not in play as root, COMPLIANCE durability was not tested below MinIO's own process, and the cross-region create/HeadBucket-301 paths never ran |
...
| F05 | P1 | Bulk governance decisions can race with another decision | ☑ | One decision service; the claim is a single `UPDATE … WHERE id=… AND status='PENDING'`, so atomicity does not rest on `FOR UPDATE`. All four surfaces (single, bulk, sample review, reviewer agent) route through it. Proven on **file-backed** SQLite and then **reproduced concurrently on real PostgreSQL 17** — 7 contention shapes × 2 isolation levels, racers released from a shared barrier on separate connections. Removing the `AND status='PENDING'` guard initially failed only 1 of 12, because a target adapter's own precondition was propping the invariant up by accident; an adapter-entry counter now makes the mutation fail 2==1 and 3==1. **Operational caveat:** at REPEATABLE READ the loser gets SQLSTATE 40001 rather than a 409, and nothing supplies a retry loop — raising the isolation level would turn lost races into 500s |
| F06 | P1 | Browser OIDC integration is incomplete | ☑ / ⚠ | **The flow is implemented and was driven in a browser against a real OIDC issuer.** `ui-next/src/lib/oidcClient.ts` runs authorization code + PKCE (`S256` via `crypto.subtle`, no new dependency): endpoints come from the issuer's discovery document, not from configuration; `state` and the verifier are generated per request and a `code` with no matching transaction is discarded, not exchanged; the exchange, the renewal and the revocation on sign-out all happen against the IdP. Access and refresh tokens stay **in memory** — only the verifier/`state`/`nonce` reach `sessionStorage`, single-use, because they must survive the redirect. The blocking screen is now a sign-in screen when an issuer is configured and the same honest explanation when one is not. `compose.oidc.yaml` (development-only overlay) runs `ghcr.io/navikt/mock-oauth2-server` and switches the API to `AIDA_IDENTITY_PROVIDER=oidc`; **`development` remains the default**. Verified in the browser through nginx on :3001 → API on :8000: fresh sign-in reaching the catalog with real data; sign-out (token dropped, refresh grant revoked, sign-in screen back); expiry (token lapsed → `/me` 401 → shell shows **Sign-in required**, shell still mounted, no fallback to the dev principal); least privilege (a token whose `roles` claim maps to Viewer got 403 on `/v1/organizations` and `…/workspaces` where the admin token got 200, and a server-derived persona from its `groups` claim). **Found by running it:** the first renewal implementation looped a token request every 2s forever when the issuer re-issued the same absolute `exp`; renewal now has to move the deadline by more than the minimum renewal interval or it stops and the session lapses. Also fixed: `OrgProvider`/`ScopeProvider` moved below the auth gate — they used to fetch before any token existed and never re-fetched after sign-in. ⚠ A mock issuer proves the **protocol integration** only. Still needs a corporate IdP: user directory, password/MFA, consent, SSO session policy and silent re-authentication after a reload, administrative revocation, and client registration of the redirect URI |
| F07 | P1 | Gateway's copied MCP endpoint fails in production topology | ☑ | Production nginx now proxies `/mcp` (prefix match, so `/mcp/` is covered; 300s timeouts for governed tool calls; buffering deliberately left on with a note to change it if a streaming transport is added). Two gates: a static contract check that fails if nginx and `vite.config.ts` disagree about API prefixes, and a CI job that runs the **real built image** against a stub upstream and asserts on response bodies — including the pre-fix negative case, which returns the SPA shell. On-screen diagnostic distinguishes reached / unauthorized / **spa-shell** / unexpected / unreachable. **Now run end to end against the live stack**: `initialize` and `tools/list` both succeed through the production nginx on :3001 into the running FastAPI; a request with no identity is refused 401; and least privilege genuinely differs through the proxy (all-roles principal sees 7 tools, a Viewer sees 6). This is the review's acceptance criterion met against the real topology, not a stub |
...
| F15 | P1 | Source/workspace pickers silently operate on capped first pages | ☑ | Both halves landed. Client: bounded paging with an explicit budget, reported `total`/`truncated`, deep-linked ids resolved by a bounded scan. Server (this pass): optional `q=` on all four list routes (`list_organizations`, `list_workspaces`, `list_organization_projects`, `list_organization_datasources`) matching name and slug case-insensitively via the catalog's own `lower(col) contains` convention, plus `GET /v1/organizations/{id}`, `/v1/workspaces/{id}`, `/v1/projects/{id}`, `/v1/datasources/{id}` so a remembered selection resolves in one request instead of a scan. `pg_trgm` GIN indexes (`c3f0a71d5e94`) measured to turn the OR-of-substrings into a BitmapAnd of index scans (cost 1906 -> 81 at 200k rows). `tests/test_scope_picker_search.py` asserts, per route, that a term matching another organization's resource returns nothing and that another organization's id is refused |
...
| F21 | P2 | Browser resilience and modal accessibility need shared infrastructure | ☑ / ⚠ | Route error boundary distinguishing a failed chunk (reload) from other errors (retry), printing the correlation id. Real `Dialog` with focus trap, focus restore, Escape and background inertness — replacing `window.prompt` on the screens being fixed. `AsyncState`, `Toast`, `ConfirmDialog`, `FormErrors`, unsaved-edit protection. Lineage graph gained a table alternative; virtual list gained keyboard traversal. **The command palette was still broken and jsdom could not see it.** Driving the running app in a real browser found focus escaping the open modal into the sidebar after 52 Tab presses — the review's own worked example, still failing after the primitive existed, because `App.tsx` never adopted it. Now fixed and re-verified in-browser: focus trapped forward (60 Tabs) and backward (25 Shift+Tabs), background `inert` + `aria-hidden` while open and released on close, and focus restored to the opener on Escape. A regression test pins the structural half in jsdom, which has no sequential focus navigation and so can never assert the Tab-order half. **An axe-core audit against the running app then found what jsdom never could**: `aria-required-children` (critical) on every catalog row, `aria-allowed-attr` on three screens, `color-contrast` on *every* screen, `button-name` shell-wide, and a scroll region no keyboard could reach. The `VirtualList` one mattered most — it carried `aria-rowcount`/`aria-rowindex` on a `role="list"`, where those attributes are invalid and **silently ignored**, so the position announcement the component exists to make was never made to anyone. The contrast failures were also a token bypass: hard-coded hexes measuring 2.59–4.13 where the token they should have used already passes. All fourteen screens scanned now report **zero** WCAG 2.1 A/AA violations against the rebuilt production image. ⚠ axe-core is an automated engine; still no screen-reader pass, so no conformance claim |
...
| R04 | Reduce centralized ORM/schema coupling | ◐ | **One context relocated, and the procedure for the rest written down** — the review's own instruction, taken literally. `profiling` (module 05) moved out of `aida.models`/`aida.schemas` into `atlas.modules.profiling`: 9 ORM models, 15 API DTOs, persistence and DTOs in separate files. `catalog` was *not* the first move because it was already relocated; profiling was named as the next owner by `atlas.modules.catalog.models`' own docstring. **Zero importer changes** — the re-export shims keep all 155/89 callers working, and the register at `40-engineering/09-compatibility-shim-register.md` counts them (286 files through `aida.models`, 18 through `aida.schemas`). New `profiling module privacy` import-linter contract, 11 kept / 0 broken. `Base.metadata` proven unchanged by a canonical before/after dump of 192 tables (columns, types, constraints, indexes, defaults, and `distinct_registries=1`) diffed mechanically, the "before" taken from a clean `git worktree`; `test_migration_orm_drift.py` ran against a real PostgreSQL, not skipped. OpenAPI **byte-identical**, and `openapi_diff.diff_specs` reports 0 changes of *any* classification, not merely no breaking ones. No migration; nothing under `migrations/versions/` touched. Procedure: `40-engineering/10-bounded-context-relocation-procedure.md`. Still ◐: 5,378 lines remain in `models.py` and 3,771 in `schemas.py`, and this context's routes, service and repository have not moved |
| R05 | Split the frontend API client by domain | ☑ | Partial: transport, identity, glossary, cross-source and column-documentation extracted behind an unchanged barrel. **It was a correctness fix, not cosmetics** — all three `_*_api.ts` modules had copied their own transport, so the F06 header rule and F14 correlation ids never reached those calls, and each reinstated the `api → org → api` cycle. `api.ts` is now **60 lines** of pure re-exports across 18 domain modules; 291 exported names identical before and after, verified with TypeScript's own checker rather than grep. Finishing it found five more raw `fetch` calls with private decoders that never reached `observeRequests`, so a failed workbook upload or description withdrawal was invisible to the shell's connection state, and a **duplicate `decideRelationshipCandidate`** — two clients for one route, previously shadowed by declaration order. The duplicate is reported, not silently resolved: choosing a winner is a behaviour change |
| R06 | Break large screens into workflow sections | ☑ | Deliberately limited to screens already being changed for a correctness fix, per the review's "correctness first, not a visual rewrite". Agent gateway 844→707 and Business meaning 990→741, each with an extracted module. The remaining five then followed: 954→583, 940→326, 893→429, 890→238, 844→275. The shared extraction that mattered was not markup but controllers — **nine hand-rolled async loaders that disagreed with each other**, three ordering responses with a monotonic ticket and the rest relying on `AbortController` alone (which cancels the request but does not order the two `setState` calls when the abort loses the race), plus nine submit controllers using `submitting` *state* as a re-entrancy guard so two submits in one tick both passed. Two screens deliberately left large, with reasons |
...
| D03 | Do not remove compatibility shims prematurely | ☑ | **Nothing deleted — the register D03 actually asks for now exists**, generated by `scripts/shim_register.py` with a `--check` gate. 14 shims, caller counts AST-measured across `src/`, `tests/`, `scripts/`, `sdk/` and `migrations/env.py` (string-constant references counted in a separate column, not folded in). Replacement path, owner area and removal condition are hand-written and labelled as such. **No shim has zero callers** — `aida.db` 221, `aida.models` 277. A discovery pass fails the gate on any shim-shaped file the register does not list |
...
| Global scope discoverability | ☑ | F15 — server `q=` and fetch-by-id landed; search is one request, not a client-side scan over loaded rows |
...
| 7 | External destination and credential inventory | ☑ | **Generated**, not hand-written: `scripts/generate_destination_inventory.py` with a `--check` gate. 41 settings (24 destinations, 17 credential references) across five distinct states. Prints setting *names* and characterizations only — never a value — enforced by two automated checks plus a manual canary grep. Settings nothing reads dropped 4 → 1 once F01's S3 provider consumed the `object_store_*` trio; the remaining one is named |
...
| Reachability review across both namespaces | ☑ | Backend gate green with two stale allow-list entries removed, and a **frontend** walker added (`scripts/check_frontend_reachability.py`, wired into CI). Its design point is the one that matters: statement-level `import type` is tracked separately and does not confer reachability, because TypeScript erases it — the first version counted it and passed clean on the pre-fix tree. Re-run against `a3072e6^` it correctly fails on `ProposalCard.tsx` as "named only by an erased `import type`". 3 unreachable modules today, all allow-listed with reasons |
...
| Domain READMEs / generated architecture map | ☑ | Five per-context domain guides written and linked from the module index, plus a **generated** architecture map (`--check` gated) that aggregates 348 modules / 1,661 edges into 12 groups by a pure function of the dotted path. It makes a checkable claim about routers versus domain modules and **reports where that claim fails** — 12 domain/platform modules still import a router, named rather than smoothed away |
...
| T05 Shared atomic review transition | F05 | ☑ (reproduced on real PostgreSQL) |
| T06 Durable notification intents | F12 | ☑ |
| T07 Complete browser authentication | F06 | ☑ against a mock issuer; ⚠ corporate IdP |
...
| T13 Searchable paginated pickers | F15 | ☑ |
...
Two items this file previously recorded as blocked were not, and one gate had silently rotted:

- **F05's PostgreSQL reproduction was never blocked** — a real PostgreSQL was available on the development machine all along. It is now done, and the mutation test showed the invariant had partly been held up by a target adapter's own precondition rather than by the claim.
- **F20's truncation banner had never rendered.** `normalized_sql` is the literal-redacted form (`LIMIT %(redacted)s`), so the regex it depended on matched nothing on any execution. Its test passed only because the fixture carried an un-redacted `LIMIT`. The banner now reads `applied_row_limit`, and `row_limit_source` separates a gateway cap from the caller's own `LIMIT` — asking for 100 rows and getting 100 is not truncation.
- **Nine delivery tests were a time bomb.** They enqueued at wall-clock time and asserted against a hard-coded `2026-09-06 12:00Z`, so they passed for a few hours and then failed permanently — which is what happened mid-pass, with no code change involved. Both files are now clock-independent: fixed, ordered instants where the production call accepts a clock, and a per-call sample where it deliberately does not.
- **D04 regressed and was re-fixed.** A `git reset` undid the `git rm --cached`, and `.gitignore` does not apply to already-tracked files, so all 60 Vitest artifacts returned. Re-untracked, with `git check-ignore` confirming the rule now matches.
...
- A **corporate** IdP for F06. The redirect, the PKCE exchange, renewal, revocation, expiry and role/persona mapping are now implemented and were driven in a browser against `ghcr.io/navikt/mock-oauth2-server` through the production nginx topology (`compose.oidc.yaml`). What that cannot show: a user directory, passwords or MFA, consent, an SSO session cookie (so silent re-authentication after a page reload is untested — today a reload returns to the sign-in screen), administrative token revocation, and redirect-URI registration, which the mock does not enforce.
- An **AWS** bucket, to confirm F01 against the real service rather than MinIO — the behaviours that differ are listed on the F01 row.
- A real SOC collector, for F04.
- A **deployed** topology (not the local compose stack), for fresh-browser auth against a real IdP, disaster recovery and T28. The local stack has now closed what it can: the full JSON-RPC path through production nginx, and the browser-verified focus behaviour on F21.
- A **screen reader**, before any accessibility conformance is claimed. Automated WCAG 2.1 A/AA is now clean on every screen scanned, which is a floor and not conformance.
