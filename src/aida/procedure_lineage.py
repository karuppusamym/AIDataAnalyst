...
loop's per-row processing, and data that flows through a variable or a loop
record, is not modeled beyond its own statements; `TRY`/`CATCH` error-handling
...
* **Cursor reads.** `OPEN c FOR <query>` (PL/SQL, PL/pgSQL; `FOR <string>` and
  ...
  routine-local state. Their rows never claim the routine's result set: an OUT
  ref cursor is not told apart from a local one.
...
_OPEN_FOR_RE = re.compile(
    r"^\s*OPEN\s+:?[A-Za-z_][\w$#.]*\s+(?:(?:NO\s+)?SCROLL\s+)?FOR\b\s*", re.IGNORECASE
)
...
def _resolve_owners(
    ...
    resolve_column_owners(
        node, aliases=_collect_table_aliases_with_temp(node, subject)[0], declared=names
    )
...
def _classify_chunk(
    ...
    results.extend(
        ...
        for statement in _classify_and_extract(
            ordinal, peeled, dialect, sqlglot_dialect, plpgsql, subject, names
        )
    )
    return results
...
def _query_into_local(
    ...
    *,
    ...
    """`query` -- the tail of a cursor's declaration or OPEN -- read into routine-local
    state, or the gap that says why it could not be. `what` names the shape for a
    query that parses as something else; it is fixed text, never the body's."""
    if not _SQLGLOT_AVAILABLE:
        return _unparsed_statement(
            ordinal, dialect, context,
            f"{UnparsedReason.PARSE_ERROR.value}: sqlglot library is not available",
...
def _open_for(
    ...
    The rows go to `PROCEDURE_LOCAL_TARGET`, as a declared cursor's do. A ref cursor
    passed back through an OUT parameter hands them to the caller; this parse does not
    tell that cursor from a local one, so it never claims the routine's result set --
    the read is what it records, and the read is what was dropped."""
...
def _emit(
    ...
    function it reads; return the next free ordinal."""
    statement = _tokens_located(parsed, context)
...
def _walk_unit(
    ...
    context = replace(context, names=context.names.with_names(_unit_declared_names(sql, unit)))
    statements: list[ParsedStatement] = []
    for item in unit.declarations:
...
def _walk(sql: str, dialect: str, subject: Mapping[str, str] | None) -> _Walk:
    ...
    context = _WalkContext(
        ...
        # header's parameters -- which `_consume_subprogram` does not read for a
        # schema-qualified name (`PROCEDURE ops.p(x ...)` reads as a unit named `ops`).
        names=(
    ...
    if block is not None:
        ...
        context = replace(context, names=names)
        start, end = _extract_body_span(sql, dialect)
