...
async def resolve_plan_tools(
    session: AsyncSession, plan: ToolPlan, context: SecurityContext,
    ...
    for step in plan.steps:
        try:
            ...
            issues.append(ValidationIssue(step.sequence, "Select a published tool and version", "ERROR"))
            continue
        version = await session.scalar(select(GovernedToolVersion).where(
        ...
        if version is None:
            issues.append(ValidationIssue(step.sequence, "Tool version is unavailable or unpublished", "ERROR"))
            continue
        if "PlatformAdmin" not in context.roles and (
            ...
            issues.append(ValidationIssue(step.sequence, "Tool execution permission denied", "ERROR"))
            continue
        ...
        try:
            render_tool_sql(
                version.sql_template, dialect=datasource.dialect,
                definitions=[ToolParameterDefinition.model_validate(p) for p in version.parameter_schema],
                values=step.parameters,
            )
        except (ToolParameterError, HTTPException):
            issues.append(ValidationIssue(step.sequence, "Parameters do not satisfy the tool contract", "ERROR"))
            continue
