"""The unified-lineage reads every surface serves: REST, MCP and GraphQL (R11-GQL01).

The merged graph and the bounded impact traversal, as
`GET /v1/datasources/{id}/unified-lineage/graph`, `/impact/{node_id}` and
`GET /v1/data-domains/{id}/unified-lineage/graph` return them. They lived beside
the router in `aida.unified_lineage_api` until GraphQL needed them too: a GraphQL
module may not import a router, and a service must not take its rules from one
(R03). The router, the MCP lineage tools and GraphQL now call these same
functions, so the surfaces cannot answer differently; `aida.unified_lineage_api`
re-exports every name it used to define, for the importers it already has.

Moved unchanged. What the graph merges, and what an edge's `evidence` may carry,
is in `aida.unified_lineage_api`'s docstring; the per-edge-kind rules are in
`aida.unified_lineage_builder`.

Access is the caller's decision: these functions take a `DataSource` (or a
`DataDomain`) the caller has already loaded and authorized, and decide nothing
...
class LineageNodeNotFoundError(ValueError):
    """Raised by the reusable payload builders (shared by the REST routes in
    `aida.unified_lineage_api`, the native MCP lineage tools in `mcp_server.py`
    and GraphQL's `lineageImpact`) when a node id is not
