"""DQ-6 follow-up: the month-end seasonal baseline, wired end-to-end into the
same `evaluate_analysis_run` -> `DataQualityObservation`/`DataQualityIncident`
path DQ-1's notification routing and DQ-3's runtime coupling already consume,
unchanged -- exactly DQ-6's own wiring precedent (`tests/test_quality_seasonality_wiring.py`).

`tests/test_data_quality_month_end_seasonality.py` proves the pure comparison
function (`data_quality.day_of_month_baseline` / `evaluate_quality`) reduces
false positives on a synthetic month-end-close cycle. This file proves the
*wiring*: against a real (in-memory sqlite) database, seeded through the ORM,
with genuinely persisted `TableProfile` rows (real timestamps, real row counts,
inserted one scan at a time exactly as the profiling workflow would) --

  * with `quality_seasonal_month_end_enabled` off (the default),
    `evaluate_analysis_run` opens a VOLUME_CHANGE incident the moment a table
    enters its normal month-end close window, because it only ever compares to
    the single most recent prior profile (the ordinary day before) -- the same
    false positive the pure-function test demonstrates, now observed through
    the real incident-creation call site;
  * with the flag on, `evaluate_analysis_run`, reading that table's own
    ...
    `threshold_strategy: SEASONAL_MONTH_END`, so the reason is auditable, not
    just an absence of an incident.
"""
...
_CLOSE_WINDOW_DAYS = 2
# Jan 1 2026 .. Jun 28 2026 (five real month-end close cycles); Jun 29 2026 is
# the "current" run, June's month-end close window's first day.
_HISTORY_DAYS = 179
_CURRENT_OFFSET = 179
...
class _Scenario:
    ...
    `table_month_end_off` and `table_month_end_on` -- each carrying its own 180
    days of real, persisted month-end-close `TableProfile` history, so the two
    tables can be evaluated under different `quality_seasonal_month_end_enabled`
    settings without one run's incident state leaking into the other's.
    """
    ...
    async def build(self) -> "_Scenario":
        ...
        self.table_month_end_off = await self._table(schema, "close_batch_off")
        self.table_month_end_on = await self._table(schema, "close_batch_on")
        return self
    ...
    async def seed_history_and_get_current_run(
        ...
        for offset in range(history_days):
            self.db.add(
                TableProfile(
                    ...
                    status="COMPLETED",
                    created_at=_START + timedelta(days=offset),
                )
            )
        ...
        self.db.add(
            TableProfile(
                ...
                status="COMPLETED",
                created_at=_START + timedelta(days=current_offset),
            )
        )
        await self.db.flush()
...
async def test_default_flag_off_still_opens_the_incident_the_rolling_baseline_always_did(
    ...
    """Baseline behavior, unchanged: entering a normal month-end close window
    still trips VOLUME_CHANGE against the unmodified rolling-previous-profile
    comparison when the new flag is off (its default) -- proving the rollout is
    genuinely opt-in."""
    run_id = await scenario.seed_history_and_get_current_run(
        scenario.table_month_end_off, history_days=_HISTORY_DAYS, current_offset=_CURRENT_OFFSET
    )
    ...
    incident = await _open_volume_change_incident(db, scenario.table_month_end_off.id)
    assert incident is not None

    observation = (
        await db.scalars(
            select(DataQualityObservation).where(
                DataQualityObservation.table_id == scenario.table_month_end_off.id
            )
        )
    ).one()
...
async def test_flag_on_reads_real_persisted_history_and_suppresses_the_same_false_positive(
    ...
    """The same shape of table, same normal month-end close entry, evaluated
    with `quality_seasonal_month_end_enabled=True`: `evaluate_analysis_run`
    reads this table's own already-persisted `TableProfile` history back out of
    the database (not a mock, not a synthetic in-memory list -- real rows this
    test itself inserted earlier in the same session) and no VOLUME_CHANGE
    incident opens."""
    run_id = await scenario.seed_history_and_get_current_run(
        scenario.table_month_end_on, history_days=_HISTORY_DAYS, current_offset=_CURRENT_OFFSET
    )
    monkeypatch.setattr(
        quality_service,
        "get_settings",
        lambda: Settings(
            quality_seasonal_month_end_enabled=True,
    ...
    incident = await _open_volume_change_incident(db, scenario.table_month_end_on.id)
    assert incident is None

    observation = (
        await db.scalars(
            select(DataQualityObservation).where(
                DataQualityObservation.table_id == scenario.table_month_end_on.id
            )
        )
    ).one()
    assert observation.evidence["threshold_strategy"] == "SEASONAL_MONTH_END"
